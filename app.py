...
with col_params:
    ...
    if brand == "SIEMENS":
        ...
        c1, c2 = st.columns(2)
        with c1:
            # POS. 1-2: FAMIGLIA (Vincoli da Allegato Schneider)
            fam_opt = {
            ...
            # POS. 5: CURVA (Vincoli da Allegato Schneider)
            curv_opt = {
                "3 = Curva B": "2",  # Mappatura reale catalogo: B=2
                "4 = Curva C": "3",  # Mappatura reale catalogo: C=3
                "2 = Curva D": "5"   # Mappatura reale catalogo: D=5
            }
            curv_sel = st.selectbox("Curva (POS.5)", list(curv_opt.keys()))
            c_code = curv_opt[curv_sel]

        with c2:
            # POS. 6: POLI (Vincoli da Allegato Schneider)
            pol_opt = {
                "1 = 1P": "1",
                "2 = 2P": "2",
                "3 = 3P": "3",
            ...
            # POS. 7-8: CORRENTE NOMINALE (Vincoli da Allegato Schneider)
            amp_opt = [
        ...
        # Dati per la visualizzazione dell'analisi struttura
        pos_data = [
            ("1-2", fam_code), 
            ("3", serie_code), 
            ("4", p_code), 
