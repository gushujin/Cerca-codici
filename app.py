...
# --- SIDEBAR (Spostamento Categoria e Brand a SX) ---
...
st.caption(f"Configurazione attuale: {categoria} | Brand: {brand}")
st.divider()
...
with col_params:
    ...
    if categoria == "Magnetotermici (MCB)":
        ...
        # --- LOGICA DI GENERAZIONE CODICI BASATA SULLA TABELLA ---
        
        # Estrattori generici per pulizia stringhe
        amp = amp_val.replace("A", "")
        pol = poli_val[0] # Prende il primo numero
...
with col_res:
    ...
    if codice_final != "N/D":
        ...
        st.info(f"**Brand:** {brand}\n\n**Tag:** {pdi_val}, {poli_val}, {amp_val}, {curva_val}")
    else:
        st.warning("Configurazione non disponibile")
